...
> 搭配 `README.md`（架構/進度）一起看。最後更新：2026-06-20（對應 commit 在 `git log` 最上面）。

---

## 🆕 最新狀態（2026-06-20）· v2 新一輪規劃，issue 開完、尚未動工

**這一輪在幹嘛**：KC 找來一個參考 repo [FatihMakes/Mark-XL](https://github.com/FatihMakes/Mark-XL)（本地語音 AI 助理，Jarvis 風），由 Claude 完整解析，挑出**適合我們（串 API 架構）**的優化點。**本地大模型部署相關（Ollama、warmup、Kokoro/Vosk、GPU 設定）一律略過**。

**v1（#1–#30）已全數結案。** 這輪規劃出 **#31–#49 共 19 個 issue，已全部開好、標好 `frontend`/`backend` + `phase-0~3`，但全部尚未動工**（KC 要先把流程跟 issue 備齊）。跨前後端的 issue，**preload 契約已寫在 issue 內文**。

### 給後端同事：你的待辦（`backend` 標籤）
**先做 Phase 0 探針**（結論會影響後面怎麼做，請先查並把結論寫回對應 issue + 本檔架構事實）：
- **#31** Claw Router 走 OpenAI 端點**支不支援 prompt caching**？→ 決定 #37 要不要加快取
- **#32** Claw Router **哪些 model 吃圖片（vision）**？→ 決定 #46 多模態怎麼接
- **#33** **OS 工具安全模型** + **桌面免提 VAD 該放前端還後端**？→ 決定 #48、#41 的邊界

接著 Phase 1 後端：**#34**（長期記憶儲存層 + `save_memory` 工具 + 注入系統提示）、**#36**（工具結果可直接念時省一趟 LLM）、**#37**（系統提示靜→動排序）、**#38**（Whisper 灌領域詞）。
後續 Phase 2/3 後端：**#41**（VAD 偵測層）、**#43**（錯誤自我修復）、**#44**（背景任務佇列）、**#46**（檔案解析+多模態）、**#48**（OS 工具移植+安全，**預設關+總開關**，最大項放最後）。

### 我（前端）的待辦（`frontend` 標籤）
**#35** 記憶管理 UI · **#39** Orb 隨音量跳動 · **#40** 活動回饋即時化 · **#42** 免提語音設定 UI · **#45** 背景任務面板 · **#47** 檔案拖放區 · **#49** OS 工具總開關/個別開關 UI。（#38 Whisper 詞庫含前端小區塊。）

### 階段順序（價值高/風險低先做）
- **Phase 0 探針** #31 #32 #33（後端先）
- **Phase 1 地基·記憶+省錢** #34 #35 #36 #37 #38
- **Phase 2 體驗升級** #39 #40 #41 #42
- **Phase 3 能力擴張** #43 #44 #45 #46 #47 #48 #49

> 規矩照舊：跨前後端的 issue **先把 preload 契約敲定**再各做各的；前端沒接上後端前用 mock 降級（沿用 #23/#24 老路）。共用 main、pull-before-push、回退用 revert。

### Mark-XL 解析重點（為什麼挑這些）
- **長期記憶**（#34/#35）：它有 `save_memory` 靜默存個人事實、分類、字數上限 LRU、注入系統提示——我們完全沒有，價值最高。
- **省一趟 LLM**（#36）：它對「結果已可念」的工具跳過第二趟模型呼叫——我們有月額上限，直接省錢。
- **錯誤自我修復**（#43）：`error_handler.py` 的 retry/skip/replan/abort 決策。
- **背景任務佇列**（#44/#45）：`task_queue.py` 優先級佇列 + 取消 + 完成回呼。
- **免提 VAD**（#41/#42）：`_VADBuffer` 雙門檻遲滯，講完自動送出（補我們只有按鍵+喚醒詞）。
- **Whisper 領域詞**（#38）：我們 whisper.js 已支援 `--prompt` 但沒用。
- **UI/UX**：整體我們的 Liquid Glass 比它的霓虹 HUD 更成熟、更適合長期用；只借 3 個互動點——Orb 隨音量跳動（#39）、活動回饋即時（#40）、檔案拖放（#47）。它的視覺風格**不抄**。
- **OS 工具**（#48/#49）：它有 18 個 OS 控制工具；KC 決定**要做但預設關 + 加總開關**。
...
## 我的待辦（前端）— 2026-06-13 更新（v1，皆已完成；v2 待辦見本檔最上方 🆕 區塊）
